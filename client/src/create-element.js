...
const
	CMD_CONCAT_DIRECT = Symbol("STRIX_CMD"),
	CMD_CONCAT_TOKEN = Symbol("STRIX_CMD"),
	CMD_CONCAT_PRIMITIVE = Symbol("STRIX_CMD")
;

const resolveFragment = ([TemplateStringsArray, TemplateValuesArray, STRIX_HTML_FRAGMENT], commandBuffer) => {
	...
	TemplateValuesArray.forEach((x, i) => {
		if(Array.isArray(x) && x[2] === STRIX_HTML_FRAGMENT) {
			...
			commandBuffer.push(typeof x == "object"
				? [x, TemplateStringsArray[i + 1]]
				: [undefined, x]
			);
		}
	});
...
const createElementNew = (fragment) => {
	...
	const
		CMD_BUFFER = resolveFragment(fragment, [[undefined, fragment[0][0]]]);
		BASE_TEMP = document.createElement("div"),
		PARSER_UUID = `strix-${random(32)}`,
		CONCATTED_TEMPLATE = CMD_BUFFER
			.map(([x0, x1], i) => (typeof x0 == "object"
	...
	BASE_TEMP
		.querySelectorAll(`[${PARSER_UUID}-attr]`)
		.forEach(targetRef => {
			const ATTR_BUFFER = CMD_BUFFER[Number(targetRef.getAttribute(`${PARSER_UUID}-attr`))][0];
			Reflect.ownKeys(ATTR_BUFFER).forEach(attrIndex => {
				if(typeof attrIndex == "symbol") {
					window[attrIndex.toString()](attrIndex)(ATTR_BUFFER[attrIndex], targetRef);
				} else {
	...
	;
	BASE_TEMP
		.querySelectorAll(`[${PARSER_UUID}-ptr]`)
		.forEach(targetRef => {
			const PTR_BUFFER = CMD_BUFFER[Number(targetRef.getAttribute(`${PARSER_UUID}-ptr`))][0];
			const TEXT_BUF = new Text();
			PTR_BUFFER.watch(newValue => TEXT_BUF);
			x.replaceWith(TEXT_BUF);
		})
	;
